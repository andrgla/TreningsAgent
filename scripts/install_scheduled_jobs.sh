...
echo ""
echo "NOTE: these launchd jobs only run while this Mac is awake. The same four"
echo "      jobs also exist as GitHub Actions workflows (.github/workflows/),"
echo "      which run whether or not the Mac is on — see README 'Running in the"
echo "      cloud'. Installing both means each job runs twice."
echo "NOTE: coach-* jobs need claude-agent-sdk, the Claude Code CLI, and"
echo "      ANTHROPIC_API_KEY in .env (see .env.example)."
